...
def build_discovery(settings: HaSettings, country_code: str = "") -> DiscoveryClient:
    ...
    from app.services.socialcrawl_keys import build_pool

    # Rotating key pool: a key that runs out of credits mid-search is marked
    # exhausted and the next key takes over (managed in /admin/keys).
    pool = build_pool(get_supabase_admin())
    if not len(pool):
        raise RuntimeError("No SocialCrawl API key configured - add one in /admin/keys")
